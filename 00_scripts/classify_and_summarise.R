...
if (cur_metadata$MASK.TYPE == "state") {
  ...
  main_nat <- get_metadata("none") %>%
    pull(SOIBMAIN.PATH) %>%
    read.csv() %>%
    distinct(eBird.English.Name.2022, SoIB.Latest.Range.Status)
...
if (run_res_trends == TRUE) {
  ...
  if (run_res_trends_LTT == TRUE) {
    ...
    sens_ltt <- main %>%
      dplyr::select(eBird.English.Name.2022, SoIB.Latest.Long.Term.Status) %>%
  ...
  sens_cat <- imap(soib_year_info("cat_years"), ~ {
    ...
    sens %>%
      ...
      dplyr::select(eBird.English.Name.2022, SoIB.Latest.Current.Status.Sens) %>%
  ...
  sens_cat <- main %>%
    dplyr::select(eBird.English.Name.2022, SoIB.Latest.Current.Status) %>%
    left_join(sens_cat) %>%
...
main = main %>%
  ...
  relocate(eBird.English.Name.2022, eBird.Scientific.Name.2024, eBird.Code, Order, Family,
...
main <- main %>% 
  ...
  {if (as.numeric(str_remove(names(nat_priority)[1], "eBird.English.Name.")) == 
    ...
    dplyr::select(., -SoIB.Latest.Priority.Status) %>% 
      left_join(nat_priority, by = "eBird.English.Name.2022") 
  } else {
    .
  }} 
...
if (interannual_update == TRUE){
  ...
  if (!file.exists(status_majupd_path)) { # for the 1st interannual update in one major update cycle
    ...
    main = main %>%
      left_join(status_maj_upd, by = "eBird.English.Name.2022") %>%
      # ensuring correct order of columns
