...
if (to_run == TRUE) {
  ...
  # dir.create("01_analyses_full/trends_broken/",
  #              recursive = T)
  ...
  for (k in cur_assignment)
  {
    ...
    if(container) {
      ...
      trends_species_dir <- glue("01_analyses_full/trends_broken/species_broken_{k}")
    }
    ...
    if(container) {
      write_path <- cur_metadata %>%
        dplyr::summarise(TRENDS.PATH = glue("output/{cur_mask}/{hostname}/{k}/trends_broken_{k}.csv")) %>%
        as.character()
    } else {
      write_path <- glue("01_analyses_full/trends_broken/trends_broken_{k}.csv")
    }
    ...
    #save(data_filt, file = "data_filt_k_1.RData")
