...
source('00_scripts/00_functions_broken.R')
cur_mask <- "none"
...
trends_pathonly <- "01_analyses_full/trends_broken/"

# write paths
#lttsens_path <- "01_analyses_full/results/longterm_sensitivity_broken.RData" #cur_metadata$LTTSENS.PATH
lttsens_path <- "01_analyses_full/results/longterm_sensitivity_broken_no_mean_error.RData" #cur_metadata$LTTSENS.PATH
cursens_path <- "01_analyses_full/results/current_sensitivity_broken_no_mean_error.csv" #cur_metadata$CURSENS.PATH
#trends_outpath <- "01_analyses_full/results/trends_broken.csv" #cur_metadata$TRENDS.OUTPATH
trends_outpath <- "01_analyses_full/results/trends_broken_no_mean_error.csv"
...
trends <- list.files(path = "01_analyses_full/trends_broken/", 
...
# Some checks
trends_grfr <- trends %>% filter (COMMON.NAME == "Gray Francolin" & timegroupsf == "before 2000")

means <- trends_grfr$freq      # length 1000
ses   <- trends_grfr$se        # length 1000
B <- 20000
set.seed(2)

boot_pooled <- numeric(B)
for (b in 1:B) {
  sim_means <- rnorm(length(means), mean = means, sd = ses)
  boot_pooled[b] <- mean(sim_means)
}
pooled_mean <- mean(boot_pooled)
pooled_se   <- sd(boot_pooled)
pooled_mean; pooled_se

B <- 10000
sim_matrix <- matrix(NA, nrow = B, ncol = 1000)

for (b in 1:B) {
  sim_matrix[b, ] <- rnorm(1000, mean = means, sd = ses)
}

# sim_matrix is B x 1000
all_values <- as.vector(sim_matrix)   # flatten into a single vector

# Overall mean
mean_all <- mean(all_values)

# Overall SE
se_all <- sd(all_values)
